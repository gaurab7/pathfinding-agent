    ...
    while not open.empty():
        ...
        for neighbor in get_neighbors(current, map):
            if neighbor.type == 1:
                ...
                g = current.g + neighbor.penalty + (22 if (neighbor.x != current.x and neighbor.y != current.y) else 16) # so the condition tracks if the movement is straight or diagonal
                ...
                if not neighbor in open_set or g < neighbor.g:
                    ...
                    if (neighbor.x, neighbor.y) not in [(start.x, start.y), (goal.x, goal.y)]: 
                       color = magneta if neighbor.type == 0 else dark # a darker shade to show the different type of explored nodes
